...
CONTAINER=mbs-ubuntu-1804-x86-tmp
IMAGE=mbs-ubuntu-1804-x86
...
lxc launch ubuntu:18.04/i386 ${CONTAINER}
...
while [ ${WAIT_CNT} -lt ${WAIT_MAX} ]; do
	NETWORK_STATUS=`lxc exec ${CONTAINER} -- systemctl show --property=ActiveState network-online.target`
...
lxc publish ${CONTAINER} --alias=${IMAGE} description="This image contains the build environment for the MBS system on Ubuntu 18.04 32 bit."
